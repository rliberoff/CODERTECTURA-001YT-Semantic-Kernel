...
builder.Services.AddScoped(sp =>
{
    ...
    var kernel = new KernelBuilder()
        .WithAzureChatCompletionService(options.ChatModel, options.Endpoint.AbsoluteUri, options.Key)
        .WithAzureTextCompletionService(options.CompletionsModel, options.Endpoint.AbsoluteUri, options.Key)
        .WithAzureTextEmbeddingGenerationService(options.EmbeddingsModel, options.Endpoint.AbsoluteUri, options.Key)
        .WithLogger(logger)
        .WithMemoryStorage(memory)
        .Build();

    kernel.ImportSemanticSkillFromDirectory(Path.Combine(Directory.GetCurrentDirectory(), @"Skills"), @"AskSkill", @"SummarizeSkill");
